...
// mockups this theme was built from). Every color resolves through a CSS
// custom property (defined for both themes in globals.css) instead of a
// literal hex value, so toggling the `data-theme` attribute on <html> (see
// components/ThemeProvider.tsx) re-themes the whole app without a reload.
// Each variable stores "R G B" channel values so Tailwind's `/opacity`
// modifier (e.g. `bg-primary/50`) keeps working — see
